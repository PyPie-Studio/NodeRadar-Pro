...
- **Zero-Allocation Packet Dissection & Hex Parsing**: Use `Span<byte>`, `ReadOnlySpan<byte>`, `ReadOnlySpan<char>`, and `System.Buffers.ArrayPool<byte>.Shared` when parsing raw network frames, MAC addresses (`Span<char>` + `byte.TryParse`), mDNS headers, SSDP packets, and DHCP options. Avoid unnecessary `byte[]` or `string` heap allocations.
- **Non-Blocking Background Discovery Initialization**: Broadcast sweeps (mDNS, SSDP) must NOT block synchronous execution on the critical path of `ScanRangeAsync`. Dispatch discovery sweeps as background tasks (`Task.Run`) to prevent multi-second stalls on small IP ranges.
